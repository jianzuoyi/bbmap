...
public class RQCFilter {
	...
	RQCFilter(String[] args){
		...
		for(int i=0; i<args.length; i++){
			...
			}else if(a.equals("removechloroplast") || a.equals("chloroplast") || a.equals("chloro")){
				chloroFlag=Tools.parseBoolean(b);
			}else if(a.equals("removemito") || a.equals("mito")){
				mitoFlag=Tools.parseBoolean(b);
...
//		assert(false) : rnaArtifactFlag+"\n"+primaryArgList+"\n"+libType+"\n"+outDir;
	...
	public void process(){
		...
		if(fileListName!=null){
			...
			if((chloroFlag || mitoFlag) && chloroOutFile!=null){sb.append("chloro="+(chloroOutFile.startsWith(x) ? chloroOutFile.substring(xlen) : chloroOutFile)).append('\n');}
			if((chloroFlag || mitoFlag) && chloroStatsFile!=null){sb.append("chloroStats="+(chloroStatsFile.startsWith(x) ? chloroStatsFile.substring(xlen) : chloroStatsFile)).append('\n');}
		...
		
		{
			...
			int step=0;
			final int numSteps=(doClump ? 1 : 0)+(doSpikein ? 1 : 0)+(doFilter ? 2 : 0)+(doTrim ? 1 : 0)+(doNextera ? 1 : 0)+(riboFlag ? 1 : 0)+
					((chloroFlag || mitoFlag) ? 1 : 0)+
...
//				System.err.println("Filter. step="+step+", in="+in1+", out="+out1+", inPrefix="+inPrefix+", outPrefix="+outPrefix);
				...
				String ref0=((chloroFlag && mitoFlag) ? chloroMitoRef : chloroFlag ? chloroplastRef : mitoRef);
				String ref=taxFilter(ref0, "chloroRef.fa.gz", true);
//				System.err.println("in1z="+in1z+"\nout1z="+out1z+"\ninPrefix="+inPrefix+"\noutPrefix="+outPrefix);
				
				{
					File f=new File(ref);
					if(!f.exists() || f.length()<200){
						System.err.println("Can't find chloroplast/mito for taxa; using full dataset.");
						ref=ref0;
					}
				}
...
//			assert(false) : scafstats+", "+microbeStatsFile;
	...
	private boolean runSketch_inner(String in1, String fname, String db, boolean append){
		...
		argList.add("samplerate="+sketchSamplerate);
		...
		if(append || true){argList.add("append");} //True because db name gets written first.
		else{argList.add("ow");}
...
//		sdf.setTimeZone(TimeZone.getTimeZone("PST"));
	...
	/** Remove reads from chloroplast sequence with BBMap */
	private boolean chloroFlag=false;
	/** Remove reads from mitochondrial sequence with BBMap */
	private boolean mitoFlag=false;
	...
	private int sketchReads=1000000;
	private String sketchSamplerate="0.5";
