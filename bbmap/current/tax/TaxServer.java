...
public class TaxServer {
	...
	public TaxServer(String[] args) throws Exception {
		...
		if(treeFile!=null){
			...
			if(tree.nameMap==null && hashNames){
...
//			if(searcher.refFiles.isEmpty()){System.gc();}
		...
		hasSketches=!searcher.refFiles.isEmpty();
		if(hasSketches){
			outstream.println("Loading sketches.");
			searcher.loadReferences(1);
//			System.gc();
		}
	...
	class SketchHandler implements HttpHandler {
		...
		public void handle(HttpExchange t) throws IOException {
			...
			if(sketches==null || sketches.isEmpty()){
				...
				searcher.compare(sketches, response, params, maxConcurrentSketchThreads);
		...
		private ArrayList<Sketch> loadSketchesFromFile(String fname, DisplayParams params){
			...
			if(tool.minKeyOccuranceCount!=params.minKeyOccuranceCount){tool=new SketchTool(SketchObject.targetSketchSize, params.minKeyOccuranceCount);}
			if(verbose2){System.err.println("Loading sketches from file "+fname);}
			sketches=tool.loadSketches(fname, (SketchMakerMini)null, params.mode, params.samplerate, params.reads);
