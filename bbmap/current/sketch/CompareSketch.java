...
//		outstream.println("Executing "+getClass().getName()+" "+Arrays.toString(args)+"\n");
		...
		for(int i=0; i<args.length; i++){
			...
			else if(a.equalsIgnoreCase("nr") || (a.equals("ref") && "nr".equalsIgnoreCase(b))){
				defaultParams.dbName="nr";
				searcher.refFiles.addAll(NR_PATH());
				blacklist=Blacklist.ntBlacklist;
				if(!setK){k=9; k2=5;}
				assert(false);
			}else if(a.equalsIgnoreCase("nt") || (a.equals("ref") && "nt".equalsIgnoreCase(b))){
				defaultParams.dbName="nt";
				searcher.refFiles.addAll(NT_PATH());
				blacklist=Blacklist.ntBlacklist;
				if(!setK){k=defaultK; k2=24;}
			}else if(a.equalsIgnoreCase("refseq") || (a.equals("ref") && "refseq".equalsIgnoreCase(b))){
				defaultParams.dbName="RefSeq";
				searcher.refFiles.addAll(REFSEQ_PATH());
				blacklist=Blacklist.refseqBlacklist;
				if(!setK){k=defaultK; k2=24;}
			}else if(a.equalsIgnoreCase("silva") || a.equalsIgnoreCase("ribo") || (a.equals("ref") && ("silva".equalsIgnoreCase(b) || "ribo".equalsIgnoreCase(b)))){
//				TaxTree.SILVA_MODE=Tools.parseBoolean(b);
				defaultParams.dbName="Silva";
				searcher.refFiles.addAll(SILVA_PATH());
				blacklist=Blacklist.silvaBlacklist;
				if(!setK){k=defaultK; k2=0;}
			}else if(a.equalsIgnoreCase("img") || (a.equals("ref") && "img".equalsIgnoreCase(b))){
				defaultParams.dbName="IMG";
				searcher.refFiles.addAll(IMG_PATH());
		...
		if(in.isEmpty() && args.length>0 && !allToAll){ //Allows first argument to be used as the input file without in= flag
			String x=args[0];
			if(x.indexOf('=')<0 && new File(x).exists() && searcher.refFiles.size()>0 && x.equals(searcher.refFiles.get(0))){
				searcher.refFiles.remove(0);
				in.add(x);
			}
		}
	...
	public void process(Timer t){
		...
		searcher.makeTool(defaultParams.minKeyOccuranceCount);
		
		if(!useWhitelist || allToAll){
			if(allToAll){
				searcher.loadReferences(defaultParams.minKeyOccuranceCount);
				inSketches=(ArrayList<Sketch>) searcher.refSketches.clone();
				if(searcher.autoIndex && !makeIndex){
					makeIndex=true;
					searcher.makeIndex();
				}
			}else{
				inSketches=searcher.tool.loadSketches_MT(defaultParams.mode, defaultParams.samplerate, defaultParams.reads, in);
				...
				if(searcher.autoIndex){makeIndex=inSketches.size()>8;}
				searcher.loadReferences(defaultParams.minKeyOccuranceCount);
			}
		}else{
