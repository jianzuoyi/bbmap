...
//		}
		...
		tool=new SketchTool(targetSketchSize, defaultParams.minKeyOccuranceCount);
...
//		outstream.println("Created prefilter.");
	...
	private void spawnThreads(final ConcurrentReadInputStream cris){
		...
		if(mode==PER_TAXA || mode==PER_IMG){
			longMap=new ConcurrentHashMap<Long, SketchHeap>();
		}
...
//			System.err.println("pt.readsProcessedT="+pt.readsProcessedT);
		...
		if(ffout!=null){
			if(mode==PER_TAXA || mode==PER_IMG){
				...
				success&=writeMap(longMap);
			}else if(mode==ONE_SKETCH){
				Sketch sketch=new Sketch(singleHeap, true);
		...
		public void run(){
			...
			for(SketchHeap polledHeap=queue.poll(); polledHeap!=null; polledHeap=queue.poll()){
				if(polledHeap.sketchSizeEstimate()>0){
					Sketch s=new Sketch(polledHeap, true);
					SketchTool.write(s, tsw[tnum]);
					sketchesWrittenT++;
				}
			}
			success=true;
...
//	}
	...
	private class ProcessThread extends Thread {
		...
		ProcessThread(final ConcurrentReadInputStream cris_, final int tid_){
			...
			smm=new SketchMakerMini(tool, mode);
		}
...
//					assert(taxID>0) : imgID+", "+taxID+", "+rid; //123
				...
				if(mode==PER_SEQUENCE){
					...
					if(smm.heap==null || smm.heap.capacity()<expectedSketchSize){
						smm.heap=new SketchHeap(expectedSketchSize, defaultParams.minKeyOccuranceCount);
					}
				}else if(mode==PER_TAXA){
					if(sizeList==null){
						if(smm.heap==null){smm.heap=new SketchHeap(targetSketchSize, defaultParams.minKeyOccuranceCount);}
					}else{
			...
			synchronized(longMap){
				...
				if(old==null){
					if(expectedHeapSize==smm.heap.capacity()){//Store the current heap
						...
						SketchHeap temp=new SketchHeap(expectedHeapSize, defaultParams.minKeyOccuranceCount);
						temp.add(smm.heap);
						longMap.put(key, temp);
					}
				}else{
					old.add(smm.heap);
...
//			assert(heap.heap.size()==heap.set.size()) : heap.heap.size()+", "+heap.set.size();
	...
	private ConcurrentHashMap<Long, SketchHeap> longMap;
