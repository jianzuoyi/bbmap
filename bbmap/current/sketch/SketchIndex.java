...
import structures.AbstractIntHashMap;
...
//		System.gc();
	...
	public ArrayList<Sketch> getSketches(Sketch a, int minHits, boolean countContamHits){
		if(useIntMap){
			return getSketchesMap(a, minHits, countContamHits);
		}else{
			return getSketchesList(a, minHits, countContamHits);
		}
	}
	
	public ArrayList<Sketch> getSketchesList(Sketch a, int minHits, boolean countContamHits){
...
//			System.err.println(Arrays.toString(ids));
			if(ids!=null && ids[0]>=0){
				...
				for(int id : ids){
					if(id>=0){
						idList.add(id-1);//Minimum id is 1, indicating sketch 0.
						incr++;
					}
				}
...
//		System.err.println("idList.size:"+idList.size);
		if(idList.size<minHits){return null;}
		idList.sort();
...
//	static ThreadLocal<IntHashMap> intMapHolder=new ThreadLocal<IntHashMap>();
	...
	public ArrayList<Sketch> getSketchesMap(Sketch a, int minHits, boolean countContamHits){
		
		final Timer t=(printTime ? new Timer() : null);
		
		final int[] singleton=new int[1];
		
//		IntHashMap idMap=intMapHolder.get();
//		if(idMap==null){
//			idMap=new IntHashMap(Tools.min(size, indexLimit, intMapSize), 0.7f);
//			intMapHolder.set(idMap);
//		}else{
//			assert(false);
//			idMap.clear();
//		}

//		final AbstractIntHashMap idMap;
//		if(useIntMapBinary){
//			idMap=new IntHashMapBinary(Tools.min(size, indexLimit, intMapSize), 0.7f);
//		}else{
//			idMap=new IntHashMap(Tools.min(size, indexLimit, intMapSize), 0.7f);
//		}
