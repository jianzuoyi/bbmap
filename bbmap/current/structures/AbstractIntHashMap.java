...
//			Shared.printMemory();
	...
	public final String toStringSetView(){
		...
		final int[] koys=keys(), volues=values();
		StringBuilder sb=new StringBuilder();
		sb.append('[');
		String comma="";
		for(int i=0; i<size; i++){
			if(koys[i]!=invalid){
				sb.append(comma+"("+i+", "+koys[i]+", "+volues[i]+")");
				comma=", ";
			}
		}
		sb.append(']');
	...
	public final boolean verify(){
		...
		int numValues=0;
		int numFound=0;
		if(koys.length!=volues.length){return false;}
		for(int i=0; i<koys.length; i++){
			final int key=koys[i];
			if(key==invalid){
				if(volues[i]!=0){return false;}
			}else{
				numValues++;
