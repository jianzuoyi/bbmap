...
color=family        Color records at the family level.  color=f will disable.
                    ...
                    writing to a file and t if writing to stdout.  Requires
                    the taxtree to be loaded.
...
useImgAsName        Use IMG id in the name field of format 3.
useTaxidAsName      Use NCBI TaxID in the name field of format 3.
useTaxAsName        Use taxonomic name in the name field of format 3.
...
comparesketch() {
	...
	local CMD="java $EA $z $z2 -cp $CP sketch.CompareSketch $@"
#	echo $CMD >&2
	eval $CMD
}
