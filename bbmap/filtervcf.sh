#!/bin/bash
#filtervcf in=<file.vcf> out=<file.vcf>
...
Filtering by optional attributes (such as allele frequency) require VCF files
...
filtervcf() {
	...
	local CMD="java $EA $z $z2 -cp $CP var2.FilterVCF $@"
	echo $CMD >&2
	eval $CMD
}
